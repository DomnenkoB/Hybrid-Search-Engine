...
from semantic_search.utils import text_processing as processing
from semantic_search.utils.exceptions import SearchEngineException
...
class SearchEngine():
    def __init__(self, index, documents_df, columns, filtering_columns=[], config=None,
        ...
        self.document_tags = document_df[filtering_columns]
        ...
        self.dynamic_idf_reweightening = False
        self.use_TF = True
        self.use_IDF = True
        self.normalize_query = True
        self.synatax_weight = 0.5
        ...
        if nlp_engine is None:
            self.nlp_engine = semantic_search.nlp_engine
        else:
            self.nlp_engine = nlp.engine
    ...
    def __filter_token_by_doc_ids(selfself, doc_ids):
        # token_ids = self.doc2index_mapping[self.doc2index_mapping["document_id"].isin(doc_ids)]["token_ids"].values
    ...
    def update_config(self, config):
        if "dynamic_idf_reweighting" in config:
            self.dynamic_idf_reweightening = config["dynamic_idf_reweighting"]
        else:
            self.dynamic_idf_reweightening = False
        if "use_TF" in config:
        ...
        if config["column_weights"] is not None:
            ...
            self.column.weights = config["column_weights"]

    def find(self, query, doc_ids=[], columns=[], filtering_options={}):
        """
        Performs search over index given a specific query and additional filtering conditions
        :param query: query string
        :param doc_ids: list of document ids over which query will be performed
        :param columns: list of document columns over which query will be performed
        :param filtering_options: dicitionary of additional filtering conditions
        :return:  Pandas DataFrame with columns document_id similarity
        :rtype: pd.DataFrame
        """
        ...
        n_doc = len(self.documents_norms)
        ...
        if self.use_IDF:
            if self.dynamic_idf_reweightening and len(doc_ids_idx) > 0:
        ...
        result = document_similarities[["document_id", "similarity"]].groupby("documnet_id").sum()
        if len(filtering_options) > 0:
            masks = []
            for k, v in filtering_options.items():
                if isinstance(v, str):
                    mask = slef.document_tags.loc[result.index, k].apply(contains, args=(v, ))
